...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            window.location.href = '/admin/add-artikel';
        })

        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            window.location.href = '/admin/edit-artikel/' + $(this).attr('data-uuid');
        })

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/delete-artikel/' + $(this).attr('data-uuid');
            let label = $(this).attr('data-label');
            control.ajaxDelete(url, label)
        })

        $(document).on('click', ".submit-komentar", function(e) {
            e.preventDefault();
        ...
        $(document).on('click', '.button-publikasi', function(e) {
            e.preventDefault();
            let label = $(this).attr('data-label');

            $.ajax({
                type: 'GET',
                url: '/admin/publikasi-artikel/' + $(this).attr('data-uuid'),
                contentType: false,
                processData: false,
                success: function(response) {
                    $(".text-danger").html("");
            ...
            $('#kt_table_data').DataTable({
                ...
                columns: [{
                    ...
                    render: function(data, type, row) {
                        ...
                        return data ? tanggalPublikasi : 'Belum Terpublikasi';
                    }
                }, {
                    data: 'author',
                    className: 'text-center',
                    render: function(data, type, row, meta) {
                        return 'Di buat oleh : ' + data;
                    }
                }, {
                    data: 'uuid',
                }],
                    ...
                    render: function(data, type, full, meta) {
                        if (full.publikasi) {
                            ...
                            <div class="d-flex gap-2" style="flex-flow: wrap;">
                                ...
                                <a href="javascript:;" type="button" data-uuid="${data}" data-label="Artikel" class="btn btn-danger button-delete btn-icon btn-sm">
                                    <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                               ...
                               <button type="button" data-uuid="${data}"
                                        data-label="${full.set_artikel ? 'Unset' : 'Set'}"
                                        class="btn btn-success button-artikel btn-sm set-artikel-button"
                                        ${full.set_artikel ? '' : ''}>
                                    ${full.set_artikel ? 'Unset Artikel' : 'Set Artikel'}
                                </button>

                                <button data-bs-toggle="modal" data-bs-target="#exampleModal" type="button" data-uuid="${data}"
                                    class="flex items-center gap-2 px-5 btn text-white btn-info btn-sm">
                                    <span class="text-[10px] font-semibold">Komentar</span>
                                </button>

                                <!-- Modal -->
                                    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel${data}" aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="exampleModalLabel${data}">Komntari Artikel</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <form class="space-y-4 grid grid-cols-2 gap-1 form-data" enctype="multipart/form-data">

                                                        <input type="hidden" name="uuid_artikel" value="${data}">
                                                        <div class="mb-10 text-start">
                                                            <label class="form-label">Komentar</label>
                                                            <textarea name="komentar" id="komentar" class="form-control" cols="30" rows="10" required></textarea>
                                                            <small class="text-danger komentar_error"></small>
                                                        </div>

                                                        <div class="col-span-2 flex justify-end">
                                                            <div class="modal-footer">
                                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                                                <button type="submit" class="btn btn-primary submit-komentar">Save changes</button>
                                                            </div>
                                                        </div>
                                                    </form>
                                                </div>
                        ...
                        `;
                            ...
                            <div class="d-flex gap-2" style="flex-flow: wrap;">
                                ...
                                <a href="javascript:;" type="button" data-uuid="${data}" class="btn btn-success button-publikasi btn-sm">
                                    Publikasikan
                                </a>
                            </div>
                        `;
        ...
        function updateButtonState() {
            setTimeout(() => {
                ...
                // Cek apakah ada tombol "Unset Artikel" yang aktif
                $('.button-artikel').each(function() {
                    if ($(this).text().trim() === 'Unset Artikel') {
                        hasActive = true;
                    }
                });

                // Jika ada tombol "Unset Artikel", nonaktifkan semua tombol "Set Artikel"
                $('.button-artikel').each(function() {
                    if ($(this).text().trim() === 'Set Artikel') {
                        $(this).prop('disabled', hasActive);
                    } else {
                        $(this).prop('disabled', false);
                    }
                });
            }, 500); // Delay 500ms agar memastikan DataTable sudah selesai dimuat
        }

        $(document).on('click', '.button-artikel', function(e) {
            e.preventDefault();

            let button = $(this);
            let uuid = button.attr('data-uuid');
            let label = button.attr('data-label');

            $.ajax({
                type: 'GET',
                url: '/admin/button-artikel/' + uuid,
                contentType: false,
                processData: false,
                success: function(response) {
                    if (response.success == true) {
                        swal.fire({
                            text: `Artikel berhasil di ` + label,
                            icon: "success",
                            showConfirmButton: false,
                            timer: 1500,
                        }).then(function() {
                            initDatatable(); // Refresh tabel
